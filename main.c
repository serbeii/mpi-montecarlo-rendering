...
int main(int argc, char** argv) {
    ...
    mat4s view_matrix = glms_lookat(camera_position_global, camera_target, camera_up);
    ...
    if (rank_mpi == 0) { //Master
        ...
        SDL_Window* window = SDL_CreateWindow("MPI Monte Carlo Renderer", SCREEN_WIDTH, SCREEN_HEIGHT, 0);
        if (!window) {
            fprintf(stderr, "Window could not be created! SDL_Error: %s\n", SDL_GetError());
            SDL_Quit(); MPI_Abort(MPI_COMM_WORLD, 1); 
            return 1;
        }
        ...
        if (!renderer) {
            ...
            SDL_DestroyWindow(window); SDL_Quit(); MPI_Abort(MPI_COMM_WORLD, 1); 
            return 1;
        }

        SDL_Texture* texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STREAMING, SCREEN_WIDTH, SCREEN_HEIGHT);
        if (!texture) {
        ...
        int max_rows_per_worker = SCREEN_HEIGHT / size_mpi + (SCREEN_HEIGHT % size_mpi > 0 ? 1 : 0);
        uint8_t* recv_buffer = (uint8_t*)malloc(SCREEN_WIDTH * max_rows_per_worker * 3);
        if (!recv_buffer) {
        ...
        while (!quit) { 
            ...
            for (int i = 0; i < size_mpi; i++) {
                int start_y = i * rows_per_base_dist + (i < remainder_rows_dist ? i : remainder_rows_dist);
                int end_y = start_y + rows_per_base_dist + (i < remainder_rows_dist ? 1 : 0);

                if (i > 0) { // Send to workers
                    int params[2] = {start_y, end_y};
                    MPI_Send(params, 2, MPI_INT, i, 0, MPI_COMM_WORLD);
                } else { // Rank 0 does its part
                    if (end_y - start_y > 0) { // Only print if there are rows to process
                    ...
                    for (int y_coord = start_y; y_coord < end_y; y_coord++) { 
                        for (int x_coord = 0; x_coord < SCREEN_WIDTH; x_coord++) { 
                            ...
                            for (int s = 0; s < SAMPLES_PER_PIXEL; s++) {
                                float Px_ndc = (2.0f * ((float)x_coord + random_float_0_1()) / SCREEN_WIDTH) - 1.0f;
                                float Py_ndc = 1.0f - (2.0f * ((float)y_coord + random_float_0_1()) / SCREEN_HEIGHT);
                                float Px_cam = Px_ndc * aspect_ratio_global * tan_fov_half_global;
                                float Py_cam = Py_ndc * tan_fov_half_global;

                                vec4s ray_dir_camera_s = (vec4s){{Px_cam, Py_cam, -1.0f, 0.0f}};
                                vec4s ray_dir_world_s = glms_mat4_mulv(inv_view_matrix_global, ray_dir_camera_s);
                                Ray current_ray;
                                current_ray.origin = camera_position_global;
                                current_ray.direction = glms_normalize( (vec3s){{ray_dir_world_s.x, ray_dir_world_s.y, ray_dir_world_s.z}} );
                                
                                vec3s sample_color = trace_path(current_ray, MAX_DEPTH);
                                accumulated_color = glms_vec3_add(accumulated_color, sample_color);
                            }
                            vec3s final_color = glms_vec3_scale(accumulated_color, 1.0f / SAMPLES_PER_PIXEL);
                            ...
                            int pixel_index = (y_coord * SCREEN_WIDTH + x_coord) * 3;
            ...
            for (int i = 1; i < size_mpi; i++) {
                int start_y_recv = i * rows_per_base_recv + (i < remainder_rows_recv ? i : remainder_rows_recv);
                int end_y_recv = start_y_recv + rows_per_base_recv + (i < remainder_rows_recv ? 1 : 0);
                ...
                if (num_rows_for_worker_i > 0) {
                    ...
                    MPI_Recv(recv_buffer, chunk_size, MPI_UNSIGNED_CHAR, i, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    memcpy(pixels + start_y_recv * SCREEN_WIDTH * 3, recv_buffer, chunk_size);
                }
            }
            ...
            printf("Frame processed. Total time: %.2f seconds. Waiting for quit signal or next frame event.\n", total_render_time_end - total_render_time_start);
        ...
        free(recv_buffer); free(pixels);
        SDL_DestroyTexture(texture); SDL_DestroyRenderer(renderer); SDL_DestroyWindow(window); SDL_Quit();

    } else { // Worker processes
        int max_rows_for_this_worker = SCREEN_HEIGHT / size_mpi + (SCREEN_HEIGHT % size_mpi > 0 ? 1 : 0);
        uint8_t* worker_pixels = (uint8_t*)malloc(SCREEN_WIDTH * max_rows_for_this_worker * 3);
        if (!worker_pixels) {
        ...
        while (1) {
            ...
            int start_y = params[0]; int end_y = params[1];
            ...
            for (int y_coord = start_y; y_coord < end_y; y_coord++) { 
                for (int x_coord = 0; x_coord < SCREEN_WIDTH; x_coord++) { 
                    ...
                    int pixel_index = (local_row_idx * SCREEN_WIDTH + x_coord) * 3;
            ...
            if (num_rows_processed > 0) {
                ...
                MPI_Send(worker_pixels, chunk_size, MPI_UNSIGNED_CHAR, 0, 1, MPI_COMM_WORLD);
            }
        }
        free(worker_pixels);
    }
...
void setup_scene() {
    scene[0] = (Object){PLANE, (vec3s){{0.0f, -1.0f, 0.0f}}, GLMS_VEC3_ZERO, (vec3s){{0.0f, 1.0f, 0.0f}}, 
                       {DIFFUSE, (vec3s){{0.75f, 0.75f, 0.75f}}, 0.0f}};

    scene[1] = (Object){PLANE, (vec3s){{-3.0f, 0.0f, 0.0f}}, GLMS_VEC3_ZERO, (vec3s){{1.0f, 0.0f, 0.0f}},
                       {DIFFUSE, (vec3s){{0.9f, 0.1f, 0.1f}}, 0.0f}};

    scene[2] = (Object){PLANE, (vec3s){{0.0f, 0.0f, -3.0f}}, GLMS_VEC3_ZERO, (vec3s){{0.0f, 0.0f, 1.0f}},
                       {DIFFUSE, (vec3s){{0.1f, 0.9f, 0.1f}}, 0.0f}};

    scene[3] = (Object){CUBE, (vec3s){{0.0f, 0.0f, 0.0f}}, (vec3s){{1.0f, 1.0f, 1.0f}}, GLMS_VEC3_ZERO,
                       {METALLIC, (vec3s){{0.9f, 0.9f, 0.9f}}, 0.05f}};

    scene[4] = (Object){CUBE, (vec3s){{0.0f, 2.0f, 0.0f}}, (vec3s){{0.5f, 0.1f, 0.5f}}, GLMS_VEC3_ZERO,
...
HitInfo intersect_plane(Ray ray, Object* plane_obj) {
    ...
    if (fabsf(denom) > EPSILON) {
        ...
        if (t >= EPSILON) {
            hit_info.t = t;
            hit_info.point = glms_vec3_add(ray.origin, glms_vec3_scale(ray.direction, t));
            hit_info.normal = plane_obj->normal; // Initial normal
            hit_info.front_face = glms_vec3_dot(ray.direction, plane_obj->normal) < 0;
            if (!hit_info.front_face) {
                hit_info.normal = glms_vec3_scale(hit_info.normal, -1.0f); // Normal points against ray
            }
            hit_info.hit = true;
        }
    }
...
HitInfo intersect_cube(Ray ray, Object* cube_obj) { // AABB Intersection
    ...
    int hit_axis[2] = {-1, -1}; // To store axis of intersection for normal calc
    ...
    for (int i = 0; i < 3; i++) { // x, y, z axes
        ...
        if (inv_dir < 0.0f) { float temp = t0; t0 = t1; t1 = temp; }
        
        if (t0 > t_min) { t_min = t0; hit_axis[0] = i; }
        if (t1 < t_max) { t_max = t1; hit_axis[1] = i; } // Not strictly needed for normal if only t_min is used

        if (t_min > t_max) return hit_info; // No intersection
    }

    if (t_min >= EPSILON && t_min < INFINITY) { // Check t_min as the closer hit
        hit_info.t = t_min;
        hit_info.point = glms_vec3_add(ray.origin, glms_vec3_scale(ray.direction, t_min));
        hit_info.hit = true;

        // Calculate normal
        vec3s p_local = glms_vec3_sub(hit_info.point, cube_obj->position);
        hit_info.normal = glms_vec3_zero();
        float max_dist = -1.0f; // Find which face was hit by checking distance to center plane of face
        int best_axis = -1;

        for (int i=0; i<3; ++i) {
            float dist_to_face_center = fabsf(p_local.raw[i]);
            if (fabsf(dist_to_face_center - half_dims.raw[i]) < EPSILON * 10.0f) { // If close to face i
                best_axis = i;
                break;
            }
        }
        if (best_axis != -1) {
             hit_info.normal.raw[best_axis] = (p_local.raw[best_axis] > 0) ? 1.0f : -1.0f;
        } else { // Fallback if point is not exactly on a face (should be rare with AABB)
            // This needs robust handling; for now, use the axis associated with t_min if possible
            // This normal calculation for AABB can be tricky. A common way:
            vec3s c = glms_vec3_sub(hit_info.point, cube_obj->position);
            vec3s d = glms_vec3_divs(c, half_dims.x); // Assuming uniform half_dims for simplicity here, else component-wise
            float max_comp = 0.0; int normal_idx = 0;
            for(int i=0; i<3; i++) {
                if(fabsf(d.raw[i]) > max_comp) { max_comp = fabsf(d.raw[i]); normal_idx = i;}
            }
            hit_info.normal = glms_vec3_zero();
            hit_info.normal.raw[normal_idx] = (d.raw[normal_idx] > 0.0) ? 1.0 : -1.0;
        }
        
        hit_info.front_face = glms_vec3_dot(ray.direction, hit_info.normal) < 0;
        // AABBs are typically solid, so if ray originates inside, this normal logic needs care
        // For simplicity, we assume rays start outside.
    }
    return hit_info;
}
...
// Helper to create an orthonormal basis (tangent and bitangent) from a normal vector
void create_orthonormal_basis(const vec3s n, vec3s* b1, vec3s* b2) {
...
vec3s random_cosine_direction(const vec3s normal) {
    ...
    float sin_theta = sqrtf(1.0f - r1 * r1); // sin_theta = sqrt(1 - cos^2(theta_prime)) but cos^2(theta_prime) is r1 for this sampling
                                            // More directly: r1 is actually cos(theta_prime), so sin_theta_prime = sqrt(1-r1^2) NO
                                            // Hanrahan p.36: z' = sqrt(ξ1), so cos(theta') = sqrt(ξ1)
                                            // Using Malley's method (cosine weighted):
                                            // x' = cos(2πξ2)sqrt(ξ1)
                                            // y' = sin(2πξ2)sqrt(ξ1)
                                            // z' = sqrt(1-ξ1)
                                            // This samples points on a disk then projects to hemisphere
    float z_local = sqrtf(r1); // cos(theta_prime) where theta_prime is angle with sampled vector and local Z axis of hemisphere
    ...
    return glms_vec3_normalize(world_dir); // Should already be normalized if basis is orthonormal
}

vec3s reflect(const vec3s incident, const vec3s normal) {
    return glms_vec3_sub(incident, glms_vec3_scale(normal, 2.0f * glms_vec3_dot(incident, normal)));
}
...
vec3s trace_path(Ray ray, int depth) {
    ...
    if (!hit.hit) { // Ray missed all objects, return background/environment color
        // Simple gradient sky
    ...
    if (mat.type == EMISSIVE) {
        // For path tracing, emissive surfaces usually only contribute if hit directly by camera ray
        // or if explicitly sampled (Next Event Estimation).
        // If a path bounces and *then* hits an emissive surface, that counts as indirect light.
        // For a simple path tracer, if a ray from camera hits light, it's Le.
        // If a ray from a surface hits light, it's Li.
        return mat.color;
    }
    ...
    if (mat.type == DIFFUSE) {
        ...
        if (mat.roughness > 0.0f) {
            ...
            new_ray_dir = glms_vec3_lerp(new_ray_dir, diffuse_dir, mat.roughness);
            new_ray_dir = glms_vec3_normalize(new_ray_dir);
        }
    } else {
        return glms_vec3_zero(); // Should not happen if EMISSIVE is handled above
    }

    Ray new_ray;
    new_ray.origin = glms_vec3_add(hit.point, glms_vec3_scale(hit.normal, EPSILON * 2.0f)); // Offset to avoid self-intersection
